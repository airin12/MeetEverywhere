		...
		((Button) this.findViewById(R.id.send_button)).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!messageText.equals("")){
					TextMessage message = new TextMessage(messageText, dispatcher.getOwnData().getNickname(), dispatcher.getOwnData().getUserID(), actualConnection.getUser().getUserID());
					try {
						actualConnection.addMessage(message);
						text.setText("");
					} catch (Exception e) {
						Toast.makeText(getBaseContext(), "Wiadomo�� nie zosta�a wys�ana!", Toast.LENGTH_SHORT).show();
						try {
